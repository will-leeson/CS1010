...
def play_again(game_result, correct_word):
    # Puts the play again text on the screen.
    pygame.draw.rect(SCREEN, "white", (10, 600, 1000, 600))
    play_again_font = pygame.font.Font("assets/FreeSansBold.otf", 40)
    play_again_text = play_again_font.render("Press ENTER to Exit", True, "black")
...
def runner(wordlist, word):
    global game_result, current_guess_string, key_pressed, current_guess, guesses_count, current_letter_bg_x
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if game_result != "":
                        ...
                        if checkValidInput(current_guess_string, wordlist):
                            res = eval_guess(word, current_guess_string)

                            for i in range(5):
                                v = res[i]
                                if v == '$':
                                    current_guess[i].bg_color = GREEN
                                    for indicator in indicators:
                                        if indicator.text == current_guess[i].text.upper():
                                            indicator.bg_color = GREEN
                                            indicator.draw()
                                    current_guess[i].text_color = "white"
                                    if not game_decided:
                                        game_result = "W"

                                elif v == '*':
                                    current_guess[i].bg_color = YELLOW
                                    for indicator in indicators:
                                        if indicator.text == current_guess[i].text.upper():
                                            indicator.bg_color = YELLOW
                                            indicator.draw()
